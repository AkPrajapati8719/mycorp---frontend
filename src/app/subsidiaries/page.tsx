...
import { 
  ...
  Search, Loader2, ArrowUpRight, MapPin, Network, Cpu, TrendingUp
} from 'lucide-react';
...
export default function SubsidiaryRegistry() {
  ...
  return (
    <main className="min-h-screen bg-[#F8FAFC] text-slate-900 pt-48 pb-20 relative selection:bg-blue-100 overflow-x-hidden">
      ...
      {/* 🔮 TIER 1: CENTERED HERO SECTION (Refined for Light Theme) */}
            ...
            <AnimatePresence>
              {filteredSubs.map((sub, i) => (
                <motion.div 
                  ...
                  className="bg-white border border-slate-200 rounded-[2.5rem] p-8 group relative overflow-hidden transition-all duration-500 hover:border-blue-200 hover:shadow-2xl hover:-translate-y-2 shadow-sm"
                >
                  {/* Subtle Background Hover Light */}
                  ...
                  <div className="relative z-10">
                    <div className="flex justify-between items-start mb-10">
                      ...
                      <div className="text-right">
                        <span className="px-3 py-1 bg-green-50 text-green-600 text-[9px] font-black rounded-full border border-green-100 group-hover:bg-green-600 group-hover:text-white transition-all">
                          ACTIVE
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-5 pt-6 border-t border-slate-100">
                      ...
                      >
                         <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 group-hover/link:text-slate-900 transition-colors">Visit Node Portal</span>
                         <div className="p-2 bg-slate-50 rounded-lg group-hover/link:bg-slate-900 transition-all">
                            <ArrowUpRight size={16} className="text-slate-900 group-hover/link:text-white transition-transform" />
                         </div>
                      </a>
                    </div>
                  </div>
      ...
      {/* 🚀 TIER 3: SYSTEM CAPABILITIES CARDS (Premium Light) */}

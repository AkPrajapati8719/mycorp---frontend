...
export default function SubsidiaryRegistry() {
  ...
  const [selectedSub, setSelectedSub] = useState<any | null>(null); // 🆕 Modal State
    ...
    <main className="min-h-screen bg-[#F8FAFC] text-slate-900 pt-48 pb-20 relative selection:bg-blue-100 overflow-x-hidden">
      ...
      <section className="max-w-7xl mx-auto px-6 mb-24 relative z-10 text-center">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
          <div className="inline-flex items-center space-x-2 mb-8 bg-white border border-slate-200 px-4 py-2 rounded-full shadow-sm backdrop-blur-md">
          ...
          <h1 className="text-5xl md:text-8xl font-black tracking-tighter italic uppercase leading-[0.9] mb-8 text-slate-900">
            Global <br /> <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-500">Registry.</span>
          </h1>
          <div className="relative w-full max-w-xl mx-auto group px-2">
            ...
            <input 
              ...
              className="w-full bg-white border border-slate-200 rounded-2xl py-5 pl-14 pr-8 text-sm focus:outline-none focus:ring-4 focus:ring-blue-500/5 focus:border-blue-500 transition-all shadow-sm relative z-10"
            />
          </div>
        </motion.div>
      </section>
            ...
            <AnimatePresence>
              {filteredSubs.map((sub, i) => (
                <motion.div 
                  ...
                  onClick={() => setSelectedSub(sub)} // 🆕 Click handler
                    ...
                    <div className="space-y-5 pt-6 border-t border-slate-100">
                      ...
                      <div className="flex items-center justify-between group/link py-2">
                         ...
                         <div className="p-2 bg-slate-50 rounded-lg group-hover:bg-slate-900 transition-all">
                            <ArrowUpRight size={16} className="text-slate-900 group-hover:text-white" />
                         </div>
                      </div>
                    </div>
                  </div>
            ...
            >
              ...
              <div className="bg-slate-50 border-b border-slate-100 px-10 py-8 flex justify-between items-center">
                <div className="flex items-center gap-4">
                  ...
                  <h2 className="text-xl font-black italic uppercase tracking-tighter text-slate-900">Entity Dossier</h2>
                </div>
              ...
              <div className="p-10 space-y-10 max-h-[70vh] overflow-y-auto">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  <div>
                    <span className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Company Name</span>
                    <p className="text-2xl font-black text-slate-900 italic uppercase">{selectedSub.companyName}</p>
                    <p className="text-blue-600 text-[10px] font-black uppercase tracking-widest mt-1">{selectedSub.sector}</p>
                  </div>
                  <div>
                    <span className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Registration ID</span>
                    <p className="text-lg font-black text-slate-900 font-mono">{selectedSub.registrationId}</p>
                  </div>
                </div>

                <div className="grid grid-cols-2 md:grid-cols-3 gap-8 border-y border-slate-100 py-8">
                  <div className="flex flex-col gap-1">
                    <span className="text-[10px] font-black text-slate-400 uppercase flex items-center gap-2"><MapPin size={12}/> Headquarters</span>
                    <p className="text-xs font-bold text-slate-700">{selectedSub.hqAddress}</p>
                  </div>
                  <div className="flex flex-col gap-1">
                    <span className="text-[10px] font-black text-slate-400 uppercase flex items-center gap-2"><Calendar size={12}/> Incorporation</span>
                    <p className="text-xs font-bold text-slate-700">{selectedSub.incorporationDate || 'Verified'}</p>
                  </div>
                  <div className="flex flex-col gap-1">
                    <span className="text-[10px] font-black text-slate-400 uppercase flex items-center gap-2"><ShieldCheck size={12}/> Status</span>
                    <p className="text-xs font-black text-green-600 uppercase tracking-widest">Active Node</p>
                  </div>
                </div>

                {/* Contact Bridge */}
                <div className="bg-blue-50 border border-blue-100 rounded-3xl p-8">
                  <h4 className="text-blue-700 text-[10px] font-black uppercase tracking-[0.2em] mb-6">Uplink Protocols</h4>
                  <div className="grid md:grid-cols-2 gap-6">
                    <div className="flex items-center gap-4">
                       <div className="p-3 bg-white rounded-xl shadow-sm text-blue-600"><Mail size={18}/></div>
                       <p className="text-xs font-bold text-blue-900">{selectedSub.email}</p>
                    </div>
                    <div className="flex items-center gap-4">
              ...
              <div className="bg-slate-50 border-t border-slate-100 px-10 py-6 flex justify-end gap-4">
                <button onClick={() => window.open(`http://${selectedSub.website}`, '_blank')} className="px-8 py-3 rounded-full bg-slate-900 text-white font-black uppercase tracking-widest text-[10px] hover:bg-blue-600 transition-all flex items-center gap-2">

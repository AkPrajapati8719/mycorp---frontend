...
import { useEffect, useRef, useState } from 'react'; // UPDATED: Added useState
import { ArrowRight, Cpu, Building2, Box, Sparkles, ShieldCheck, Target, BarChart3, ArrowUpRight, Globe2, Loader2 } from 'lucide-react';
import AOS from 'aos';
import 'aos/dist/aos.css';
import { myCorpApi } from '@/src/lib/api'; // THE BRIDGE

// Master Ecosystem Data (Used as UI Blueprint and Fallback)
  ...
  useEffect(() => {
    ...
    const syncWithMainframe = async () => {
      try {
        ...
        // Update live count from PostgreSQL
    ...
    // Bioluminescent Mouse Tracking Effect
    ...
    <main ref={containerRef} className="min-h-screen bg-[#021533] text-white pt-32 pb-20 overflow-x-hidden relative selection:bg-white/10">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <section className="grid lg:grid-cols-3 gap-8 md:gap-10 w-full mb-32">
          {ECOSYSTEM_BLUEPRINT.map((sector, i) => (
            <div key={sector.slug} data-aos="fade-up" data-aos-delay={i * 150} className="w-full">
              ...
              <motion.div 
                whileHover={{ y: -10 }}
                className={`pro-glow-card relative w-full aspect-[4/5] md:h-[650px] md:aspect-auto rounded-[2.5rem] md:rounded-[4rem] overflow-hidden border border-white/10 group transition-all duration-700 h-full flex flex-col justify-between`}
                style={{
                  '--glow-color': sector.glowColor,
                } as any}
              >
                <img 
                  src={sector.img} 
                  className="absolute inset-0 w-full h-full object-cover grayscale opacity-40 group-hover:grayscale-0 group-hover:opacity-100 group-hover:scale-105 transition-all duration-1000 z-0" 
                  alt={sector.title} 
                />
                
                <div className="absolute inset-0 bg-gradient-to-t from-[#021533] via-[#021533]/60 to-transparent z-10 transition-opacity duration-700 group-hover:opacity-80" />

                <div className="absolute inset-0 z-20 p-8 md:p-12 flex flex-col justify-between h-full">
                  
                  <div className="flex justify-between items-start">
                    <div className={`${sector.accent} p-4 md:p-5 bg-white/10 backdrop-blur-md rounded-2xl md:rounded-3xl border border-white/10 transition-all duration-500 group-hover:scale-110 group-hover:bg-white/15`}>
                      {sector.icon}
                    </div>
                    <Link href={`/sectors/${sector.slug}`}>
                       <div className="w-12 h-12 rounded-full bg-white/5 backdrop-blur-md flex items-center justify-center border border-white/10 opacity-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0 transition-all duration-500 hover:bg-white/10">
                          <ArrowRight size={20} className={sector.accent} />
                       </div>
                    </Link>
                  </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex flex-wrap gap-2.5 opacity-0 group-hover:opacity-100 transition-opacity duration-700 delay-100">
                      {sector.achievements.map((ach, j) => (
                        <div key={j} className="flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-white/10 backdrop-blur-sm text-[9px] font-bold text-slate-200 uppercase tracking-widest border border-white/10 shadow-lg group-hover:border-white/20">
                          <span className={sector.accent}>{ach.icon}</span> {ach.text}
                        </div>
                      ))}
                    </div>

                    <div className="transform group-hover:-translate-y-2 transition-transform duration-500">
                      <span className={`${sector.accent} font-bold tracking-[0.3em] uppercase text-[9px] md:text-[10px] mb-3 block`}>
                        {sector.subtitle}
                      </span>
                      <h2 className="text-3xl md:text-4xl font-black italic uppercase tracking-tighter leading-none mb-4 break-words">
                        {sector.title}
                      </h2>
                    ...
                    <Link href={`/auth/register`} className="block w-full opacity-0 group-hover:opacity-100 transition-opacity duration-700 delay-300">
                      <motion.button 
                      ...
                      >
                        Register {sector.subtitle} Branch <ArrowUpRight size={14}/>
                      </motion.button>
                    </Link>
                  </div>
        ...
        <section className="max-w-4xl mx-auto text-center border-t border-white/5 pt-20 mt-10" data-aos="fade-up">
          <div className="inline-flex items-center justify-center p-5 rounded-full bg-white/5 border border-white/10 mb-8 shadow-[0_0_30px_rgba(255,255,255,0.05)]">
          ...
          <p className="text-slate-400 text-sm md:text-lg leading-relaxed max-w-2xl mx-auto">
            MyCorp's operational footprint is in a state of continuous expansion. Integrating your subsidiary into our decentralized ecosystem grants immediate access to cross-sector synergy, zero-latency resources, and the protection of the world's most advanced corporate governance framework.
          </p>
        </section>
